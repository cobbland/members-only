function indexControls(req, res) {
    ...
    if (res.locals.currentUser) {
        if (res.locals.currentUser.name) {
            title = `Hello, ${res.locals.currentUser.name}`;
            message = `Welcome, ${res.locals.currentUser.name}, to Member's Only—a simple message board.`;
        } else {
